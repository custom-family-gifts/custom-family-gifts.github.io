    ...
    <div class="customerReply">
      <div class="section">
        <div class="replyTitle"><h5>Reply:</h5></div>
          ...
          <button class="replyButton" onclick="createReplyTicket()">Submit</button>
      </div>
    </div>
  `
  return result;
...
async function createReplyTicket() {
  var replyText = $("textarea").val();
  var reply = convertReply($("textarea").val());
  API.clearCache('v2-getCustomerOrder-dev_' + window.location.search);
  if(reply && reply.length > 9) {
    $('div.customerReply').replaceWith(`
      <div class="customerReply">
        <div class="section">
          <div class="replyTitle"><h5>Reply:</h5></div>
          <div class="spinner"></div>
          <form id="reply" id="replyForm">
            <textarea class="replyText" form="replyform" placeholder="Sending reply...\n${replyText}"></textarea>
          </form>
        </div>
      </div>
    `);
    console.log("\nCALL v2-custReply: " + new Date());
    API.call({
      cacheMS: -1,
      method: 'v2-customerReplyToFreshdesk',
      httpMethod: 'POST',
      body: JSON.stringify({replyText: reply}),
      onSuccess: (data) => {
        console.log("\nv2-custReply SUCCESS: " + new Date());
        console.log("\nDATA stringified = " + JSON.stringify(data));

        if(data.solveRes.records[0].ticket_id) {
          location.reload();
        }else{
          console.log("\nNO solve response");
        }
      ...
      onFailure: (data) => {
        ...
        $('#main').html(`
          <div class="row">
            <p>An error has occured.<br>${data.error}<br>Please try again later.</p>
          </div>
        `);
      }
    });
  }else{//no reply text when submit button hit. try again
    $('div.customerReply').replaceWith(`
        <div class="customerReply">
          <div class="section">
            <div class="replyTitle"><h5>Reply:</h5></div>
            <form id="reply" id="replyForm">
              <textarea class="replyText" form="replyform" placeholder="No reply found. Enter Reply here:\n${replyText}"></textarea>
            </form>
            <button class="replyButton" onclick="createReplyTicket()">Submit</button>
          </div>
        </div>
