...
var Drawer = {
  ...
  tabs: null,
  onOpen: null,
  getData: null, // an await function
  renderExtra: null,
  init: function(drawerObj) {
    if (!drawerObj.title) throw new Error('Drawer.init() requires title (data) => {}');
    if (!drawerObj.getData) throw new Error('Drawer.init() requires data from await functionapiCall (params) => {}');
    ...
    for (var key in drawerObj) {
      this[key] = drawerObj[key];
    }
  ...
  load: async function(params) {
    ...
    if (this.renderExtra) $('#drawerExtra').append('<div class="drawerLoadingOverlay"></div>');
    try {
      this.data = await this.getData(params);
    } catch (e) {
      console.log(e);
    ...
    if (this.tabs && this.tabs.length) {
      Drawer.renderTabButtons(this.data);
      Drawer.renderTab(currentTab);

    }
    var overviewHtml = Drawer.renderOverview(this.data);
    $('#drawerOverview').html(overviewHtml);
    $('#drawerTitle').html(Drawer.title(this.data));

    if (Drawer.renderExtra && typeof Drawer.renderExtra == 'function') {
      $('#drawerExtra').html(Drawer.renderExtra(this.data))
    }
    Render.toLocalTime();
    $('#drawer').removeClass("loading");
    Drawer.loadProgressEnd();
  },
  load_old: function(params) {
    Drawer.loadProgressBegin();
    if (Modal.shown) {
      Modal.hide();
    }
  ...
  paramsToUrl: function(params) { // pushes the drawer params to url
